...
| Version | Notes |
|---|---|
| 2.3 | Six mechanisms corrected, all verified against FM Pro 26.0.1.51 (macOS): ButtonBar segment labels live in LabelCalc, not Data (Data survives round-trip byte-perfect but renders blank, a new failure class no structural audit catches, §9.1); PopoverButton labels are the exact inverse (Data, not LabelCalc); TabControl and SlideControl panel content nests inside the panel object, the sibling model withdrawn for both; slideFlagSet bit 0 hides the navigation dots; TriggerText is derived output regenerated by FM, not preserved input. Notation updated to distinguish round-trip survival from rendering. PlaceholderText one-per-paste restriction withdrawn (§31 sink-side fix covers it). Segment flag table re-derived: FM 26 uses flags="8" for every segment; active segment is ButtonBarObj segmentKey, names are attribute-only, and the legacy flag values are retired to a historical note. Per-segment icon captures confirmed icons live entirely in ButtonObj (displayType plus FNAM/GLPH/SVG stream triple) with segment Object flags untouched, and the displayType arrangement mapping was fully derived (0 label, 1 icon, 2 icon above, 3 label above, 4 icon left, 5 icon right). |
