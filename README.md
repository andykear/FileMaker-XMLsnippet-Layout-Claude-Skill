...
| Version | Notes |
|---|---|
| 2.4 | Full closure pass against FileMaker Pro 26.0.1.51. Tab Width mechanism fully decoded — `tabFlagSet` is a mode enumeration (264 Label Width, 280 + Margin, 296 Widest Label, 312 Minimum, 328 Fixed), and Fixed Width's +2pt constant confirmed via drag-free typed generation. Panel titles confirmed as `TitleCalc`-only; the prior "exports as Data" claim did not reproduce and is withdrawn. Field entry access states fully decoded across both Browse and Find modes (bits 2/24 and 4/25, one shared `CanEntryCalc`), plus a new FieldObj bit (1: select contents on entry) not previously documented. Button-level CSS placement corrected to `TextObj > Styles` — the object-level `Styles` block shown in earlier versions is silently discarded by FM and has been removed from the template. `.icon`, `.button_bar_divider`, and `.inner_border` selector mechanics corrected (real properties are `-fm-icon-color` and longhand borders, not the properties earlier versions implied). `.text` on Text objects identified as a render-inert trap (Field objects are unaffected). Conditional Formatting value-ops identified as a dialog-correct/runtime-dead trap unless `Calculation` is generated alongside `op`/`RangeBegin`. portalFlags `401` resolved as an ordinary bit combination — the "planner" label was a misattribution, not a distinct mechanism. SlideControl `Styles` optionality upgraded to confirmed. LocalCSS delta-pruning against theme defaults documented. A single selected Text object copying as plain text (not XML) documented as a methodology trap. Object flags bits 3, 9, and 16 removed as unreproduced; the legacy ButtonBar segment flag values (260/261/256/65544/65545) have been cut outright rather than kept as historical record, since no FM 26 mechanism was ever found to attach to them. |
