# FileMaker Layout XML Spec v1.1

Empirically derived from round-trip testing and analysis of 45+ production layouts across 10 real-world applications.

**v1.1 — Extended corpus analysis**
...
| 24 | 16777216 | Field input mode flag (touch/tablet input) ◎ |
| 25 | 33554432 | Decorative/non-interactive text label ○ |
...
Element order: `ToolTip` comes **before** `Bounds`. ✓  
...
| `event` | `id` | Meaning |
|---|---|---|
| `OnObjectEnter` | `1` | Field entered ✓ |
| `OnObjectExit` | `2` | Field exited ✓ |
| `OnObjectModify` | `3` | Field value modified ✓ |
| `OnObjectSave` | `4` | Field value saved/committed ✓ |

`triggerFlags="1"` on all observed instances — meaning unknown, include as-is. ◎  
`<TriggerText>` contains the script name as plain text — mirrors the `name` attribute on `<Script>`. ✓  
Multiple `<Trigger>` elements can appear within one `<ScriptTriggers>` block. ◎

---

## §19 LabelCalc

Dynamic button label. Replaces static text in `TextObj/CharacterStyleVector/Style/Data`. ✓

```xml
<Object type="Button" key="1" LabelKey="0" flags="0" rotation="0">
  <Bounds top="10" left="10" bottom="40" right="200"/>
  <TextObj flags="0">
    <Styles>
      <ThemeName>com.filemaker.theme.apex_blue</ThemeName>
    </Styles>
  ...
  <ButtonObj ...>...</ButtonObj>
  <LabelCalc>
    <Calculation><![CDATA["Save & Close"]]></Calculation>
  </LabelCalc>
  <Styles>
    <ThemeName>com.filemaker.theme.apex_blue</ThemeName>
  </Styles>
...
`LabelCalc` goes **after** `ButtonObj`, before `Styles`. ◎  
Supports full FM expressions including conditional logic:
```xml
<LabelCalc>
  <Calculation><![CDATA[Case($$employee_id = employees::id; "My Time"; Left(employees::name;1) & "'s Time")]]></Calculation>
</LabelCalc>
```
When `LabelCalc` is present, `TextObj/Data` content is ignored — leave `<Data></Data>` empty. ◎
